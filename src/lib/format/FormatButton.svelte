<script lang="ts">
    ...
    const { editableContext, createEditableHandlerWithParams, getDefaultButtonClass } =
        useEditableContext();
    ...
    const uniqueId =
        id ?? generateButtonId(`Align${format.charAt(0).toUpperCase() + format.slice(1)}`);
    ...
    const svgPaths = {
        ...
        highlight:
            'M9 19.2H5.5c-.3 0-.5-.2-.5-.5V16c0-.2.2-.4.5-.4h13c.3 0 .5.2.5.4v2.7c0 .3-.2.5-.5.5H18m-6-1 1.4 1.8h.2l1.4-1.7m-7-5.4L12 4c0-.1 0-.1 0 0l4 8.8m-6-2.7h4m-7 2.7h2.5m5 0H17',
        ...
        subscript:
            'M19.9999 21h-4v-.5c1.0989-1.0329 3.75-2.5 3.75-3.5v-1.0001c0-.5523-.4477-.9999-1-.9999h-1.75c-.5523 0-1 .4477-1 1M3.99986 6l9.26894 11.5765M13.1219 6 3.85303 17.5765',
        superscript:
