<script lang="ts">
    ...
    const uniqueId =
        id ?? generateButtonId(`Image${format.charAt(0).toUpperCase() + format.slice(1)}`);
...
<button onclick={handleClick} {...restProps} id={uniqueId} type="button" class={btnCls}>
    ...
    >
        {#if typeof svgPaths[format] === 'string'}
            <path fill-rule="evenodd" d={svgPaths[format]} clip-rule="evenodd"></path>
        {:else}
            <path fill-rule="evenodd" d={svgPaths[format][0]} clip-rule="evenodd"></path>
            <path fill-rule="evenodd" d={svgPaths[format][1]} clip-rule="evenodd"></path>
        {/if}
    </svg>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <Label for="height" class="space-y-2">
                        ...
                        <Input
                            ...
                            name="height"
                            id={heightId}
                            bind:value={videoOptions.height}
                        />
                    </Label>
                </div>
