...
<div
    class={cn(
        'character-count flex items-center gap-2 text-sm ',
        isAtLimit && 'character-count--warning text-amber-600 dark:text-amber-400',
        isOverLimit && 'character-count--error text-red-600 dark:text-red-400',
        className
    )}
>
    {#if showCircle}
        <svg height="20" width="20" viewBox="0 0 20 20" class="flex-shrink-0">
            <!-- Background circle -->
            <circle r="10" cx="10" cy="10" fill="#e9ecef" class="dark:fill-gray-600"></circle>
            <!-- Progress circle -->
            <circle
                r="5"
                cx="10"
                ...
                class={cn(
                    ...
                    isOverLimit && 'stroke-red-500 dark:stroke-red-400',
                    isAtLimit && !isOverLimit && 'stroke-amber-500 dark:stroke-amber-400'
                )}
            ></circle>
            <!-- Inner white circle -->
            <circle r="6" cx="10" cy="10" fill="white" class="dark:fill-gray-800"></circle>
        </svg>
    {/if}
