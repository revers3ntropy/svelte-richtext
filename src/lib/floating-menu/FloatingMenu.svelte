...
<div class="floating-menu" data-testid="floating-menu">
    ...
    {#if !headingsOnly && showTasks}
        <button onclick={() => runTaskCommand(editor, 'toggle')} title="Task List" type="button">
            Tasks
        </button>
    {/if}

    {#if !headingsOnly && showTable}
        <button onclick={() => runTableCommand(editor, 'table')} title="Insert Table" type="button">
            Table
        </button>
    {/if}

    {#if !headingsOnly && showImage}
        <button onclick={() => runImageCommand(editor, 'basic')} title="Insert Image" type="button">
            Image
        </button>
    {/if}
</div>
